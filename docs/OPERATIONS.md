...
**Secrets you need:** `DASHBOARD_PASSWORD` (operator Bearer for the dashboard and operator endpoints) and `CRON_SECRET` (Bearer for cron routes). Never paste either into a shared command or commit.
...
2. **Did the run finish?** `llm-history` runs (§1.3). `running` forever = killed at 800 s. A cold run processes every event group since the war start, then geocodes sequentially at 1 req/s, and writes `events:llm:v3` **once at the very end** — a timeout loses everything and the next night starts cold again. This is an open defect; until it is fixed, watch the first cold run to completion.
3. **Did the write land?** The terminal write is a silent-fail `cacheSetSafe` with a 2 s timeout. Confirm in Upstash (`EXISTS events:llm:v3`) or `curl -s $BASE/api/events | jq '[.data[]|select(.data.llmProcessed)]|length'`.
4. **All batches null?** Look at `calls` in `llm-history` for the error text. 401 = NIM key revoked. 404 = model id retired from the NIM catalog (default `qwen/qwen3.5-397b-a17b` in `server/lib/freeClaudeRouter.ts`; override with env `V3_PRIMARY_MODEL`). Also check that a stray `V3_BAKEOFF_MODEL` is **not** set in Vercel — it silently overrides the production model.
...
- **Mechanics:** in-process limiter of 40 calls/min; 3 attempts per batch with 2 s / 8 s / 32 s backoff ± 500 ms (code constants); circuit breaker pauses NIM for 5 min when > 30 % of the last 10 calls fail. With a single provider, a breaker pause makes the rest of the queue fail instantly — one burst can void a whole run. Nothing is user-visible (raw GDELT fallback).
- **Levers (Vercel env, then redeploy):** `LLM_V3_CONCURRENCY` (default 12; try 8, or 1 for fully sequential) · `LLM_BATCH_SIZE` (default 2 groups per call) · `LLM_BATCH_TIMEOUT_MS` (default 120000; a batch over this is dead-lettered and the run continues). Then force-trigger (§4.1). NIM throttles per second, not per minute, and its behaviour changes month to month; these defaults are defensive, not fitted.
